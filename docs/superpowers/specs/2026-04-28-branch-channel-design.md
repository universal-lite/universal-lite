...
- `rawhide` for checking whether the currently shipped stable payload breaks on the next Fedora base.
...
`rawhide` is created from `main`. It keeps the stable Universal-Lite payload but changes the base image to the Universal Blue rawhide-equivalent base. It publishes:

- `quay.io/noitatsidem/universal-lite:rawhide`
- `quay.io/noitatsidem/universal-lite:rawhide.YYYYMMDD`
...
The `rawhide` branch should not include DX or testing changes by default. Its purpose is compatibility signal: if the rawhide image fails, the likely cause should be Fedora/rawhide or base-image drift, not unrelated developer-mode or WIP changes.
...
6. After the scheduled `main` build succeeds, automation also syncs `main` into `rawhide`.
7. A clean sync pushes to `rawhide`, which triggers the `rawhide` image build.
...
The `rawhide` stream is not part of the user-facing promotion ladder. It is a side canary based on `main`, so it can build in parallel with the `main -> dx -> testing` cascade after `main` succeeds.
...
- A failed `main -> rawhide` sync creates a PR targeting `rawhide`.
...
The container image workflow should build on pushes to `main`, `dx`, `testing`, and `rawhide`. It should publish and sign images only for those protected stream branches, not for pull requests.
...
- `rawhide` uses the `rawhide` tag family.
...
- `rawhide` builds and publishes `rawhide` after a clean `main -> rawhide` sync.
- Manual disk builds can choose `latest`, `dx`, `testing`, or `rawhide`.
...
- Feeding rawhide changes back into `main`, `dx`, or `testing` automatically.
